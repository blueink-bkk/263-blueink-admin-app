...
//    this.catSet = new Set();
  ...
  write_cache_Obsolete() {
    writeJsonFile.sync(path.join('', 'indexp.json'), this.indexp)
    console.log(`@45 done writing cache <indexp.json>`)
    const text = yaml.dump(this.indexp,{})
    fs.writeFileSync(path.join('', 'indexp.yaml'), text, 'utf8')
    console.log(`@47 done writing cache <indexp.yaml>`)
  }
...
CatalogBuilder2.prototype.write_cache = function(fpath) {
  const cb = this;
  writeJsonFile.sync(fpath, cb.indexp)
//  console.log(`--------------------------------------------------------`)
//  console.log(cb.indexp)
//  console.log(`--------------------------------------------------------`)
  console.log(`@70 done writing cache <${fpath}>`)
}
...
CatalogBuilder2.prototype.load_cache = async function(o) {
  const cb = this;
  cb.runCount =0;
...
CatalogBuilder2.prototype.add_product_to_catIndex = function (xid) {
  const cb = this;
...
CatalogBuilder2.prototype.mk_iframe = async function(o) {
  ...
  if (xid_req) {
    console.log(`@274 mk_iframe for xid:${xid_req}`)
    const {meta,data} = await open_first_product({xid:xid_req, lang})
    // console.log(`@294 catNo:${catNo}`)
    try {
...
//    assert(cb.runCount <=0, 'fatal@297')
    ...
    //console.log({meta});
    ;(cb.verbose >0) && console.log(`@307 catNo:=${catNo}`,cb.catIndex,cb.catSet)
...
//    catNo = cb.catSet.values().next().value;
  ...
  ;(verbose >1) && console.log(`>> mk_iframe catNo:${catNo} runCount:${cb.runCount}`,o)
...
CatalogBuilder2.prototype.open_product = async function(o) {
  const cb = this;
  const {xid,lang} = o||{};
...
CatalogBuilder2.prototype.apply_template_product_blueink = function(meta, data, lang) {
  const cb = this;
...
CatalogBuilder2.prototype.mk_catalogs_for_xid = async function (o) {
  const cb = this;
  const {xid, lang} = o||{}
...
CatalogBuilder2.prototype.mk_all_catalogs = async function(o) {
  const cb = this;
  const {lang} = o||{};
...
CatalogBuilder2.prototype.rebuild_web_page = async function (p1, p2) {
  const cb = this;
  if (typeof p1 == 'string') {
    const _p1 = p1;
